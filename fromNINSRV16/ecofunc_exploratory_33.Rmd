---
title: "INLA for ECOFUNC - 3x3km sample"
...
```{r, message=FALSE}
input_data <- read.csv("/home/zofie.cimburova/ECOFUNC/DATA/OBSERVATIONS/observations_33sample.csv")
```
...
names(input_data)[names(input_data)=="bio12_worldclim_10m"] <- "bio12"
names(input_data)[names(input_data)=="bio15_worldclim_10m"] <- "bio15"
names(input_data)[names(input_data)=="bio18_worldclim_10m"] <- "bio18"
names(input_data)[names(input_data)=="bio19_worldclim_10m"] <- "bio19"
...
names(input_data)[names(input_data)=="dem_10m_nosefi"] <- "height"

pred.names <- names(input_data)[c(4:7,16:(ncol(input_data)-3))]
```
...
```
```{r, echo=FALSE}
ggplot() + geom_tile(data=input_data_na_rm, aes(x=x, y=y, fill=lc)) + coord_equal()
```
...
for (predictor in pred.names) {
  data.cor.coef[i,] <- abs(cor(input_data_na_rm.scaled$lc, input_data_na_rm.scaled[,predictor]))
...
```{r, message=FALSE}
data.cor.coef2 <- data.frame(cor.coef=double())
i = 1
for (predictor1 in pred.names) {
  for (predictor2 in pred.names) {
    cor.coef <- abs(cor(input_data_na_rm.scaled$lc, input_data_na_rm.scaled[,predictor1]*input_data_na_rm.scaled[,predictor2]))
    if (cor.coef > 0.2) {
      data.cor.coef2[i,] <- cor.coef
      row.names(data.cor.coef2)[i]<- paste(predictor1,predictor2,sep = "_")
      i = i+1
    }
  }
}

data.cor.coef2$predictor <- rownames(data.cor.coef2)
data.cor.coef2 <- data.cor.coef2[order(data.cor.coef2$cor.coef),] 
data.cor.coef2$predictor <- factor(data.cor.coef2$predictor, levels = data.cor.coef2$predictor[order(data.cor.coef2$cor.coef)])
```
```{r, echo=FALSE}
ggplot(data.cor.coef2, aes(predictor, cor.coef)) + geom_col() + coord_flip()
```
