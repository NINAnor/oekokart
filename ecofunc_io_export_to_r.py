...
def main():
    r_landcover = 'landcover_forest_open_fenoscandia_10m@g_LandCover_Fenoscandia'
    ...
    # patch first raster
    r_mountain_areas = 'mountain_areas_50m'
    ...
    # crop computation to mountain areas
    ...
    # exclude cells with coverage less than 50 % = 5000 cells
    ...
    # compute proportion of forest to open land in 1000 m cells
    r_landcover_avg = 'landcover_forest_open_fenoscandia_avg_1000m'
    #grass.run_command('r.resamp.stats', input=r_landcover, 
    #                  output=r_landcover_avg, overwrite=True)

    # create 1x1 km grid of predictors
    predictors = ['dem_10m_nosefi_float_aspect@g_Elevation_Fenoscandia',
                  ...
                  'bio12_eurolst_10m@g_Meteorology_Fenoscandia',
                  'bio15_eurolst_10m@g_Meteorology_Fenoscandia',
                  'bio18_eurolst_10m@g_Meteorology_Fenoscandia',
                  'bio19_eurolst_10m@g_Meteorology_Fenoscandia',
    ...
    for predictor in predictors:
        ...
        # compute stdev of predictor in 1 km cell
        r_predictor_std = 'temp_{}_std'.format(predictor.split('@', 1)[0])
        grass.run_command('r.resamp.stats', input=predictor,
                          output=r_predictor_std, method='stddev',
                          overwrite=True)
        
        # create column for predictor in attribute table
        # (! columns are created in lower case)
        column_med = '{}_med'.format(predictor.split('@', 1)[0])
        column_std = '{}_std'.format(predictor.split('@', 1)[0])

        column_med = str.lower(column_med)
        column_std = str.lower(column_std)

        grass.run_command('v.db.addcolumn', map=v_sample_grid_clip,
                      columns='{} double precision'.format(column_med))
        grass.run_command('v.db.addcolumn', map=v_sample_grid_clip,
                      columns='{} double precision'.format(column_std))
        ...
        grass.run_command('v.what.rast', map=v_sample_grid_clip, 
                          raster=r_predictor_std, column=column_std)
    
    # create column for response
    grass.run_command('v.db.addcolumn', map=v_sample_grid_clip,
                      columns='lc double precision')
    
    # populate column with response value
    grass.run_command('v.what.rast', map=v_sample_grid_clip, 
                      raster=r_landcover_avg, column='lc')
    ...
    table_variables = '/data/home/zofie.cimburova/ECOFUNC/DATA/variables_avg2'
